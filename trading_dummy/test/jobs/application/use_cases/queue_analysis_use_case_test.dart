...
void main() {
  group('QueueAnalysisUseCase', () {
    ...
    setUp(() {
      ...
      when(() => mockPollingService.startPollingForRun(any(), any())).thenAnswer((_) async {});
    });
    ...
    group('Successful execution', () {
      test('should execute local-first flow successfully', () async {
        ...
        verify(() => mockPollingService.startPollingForRun('run-123', 'thread-123')).called(1);
      });
      ...
      test('should save local record immediately for instant UI feedback', () async {
        ...
        expect(firstSave.id, equals('test-uuid-123'));
    ...
    group('Duplicate job detection', () {
      test('should reject duplicate pending analysis', () async {
        // Arrange
        const ticker = 'AAPL';
        const tradeDate = '2024-01-15';
        ...
        expect(
          ...
          throwsA(isA<DuplicateJobException>().having(
            (e) => e.message,
            'message',
            'An analysis for AAPL on 2024-01-15 is already pending',
          )),
        );
      });

      test('should reject duplicate running analysis', () async {
        // Arrange
        const ticker = 'TSLA';
        const tradeDate = '2024-01-20';
        ...
        expect(
          ...
          throwsA(isA<DuplicateJobException>().having(
            (e) => e.message,
            'message',
            'An analysis for TSLA on 2024-01-20 is already running',
          )),
        );
      });

      test('should allow duplicate completed analysis', () async {
        // Arrange
        const ticker = 'GOOGL';
        const tradeDate = '2024-01-25';
