...
def create_research_manager(llm, memory, config: Dict = None):
    ...
    async def research_manager_node(state: AgentState) -> AgentState:
        ...
        trace_id = state.get("trace_id", "default")
...
"""
        ...
        # CRITICAL FIX: Force consensus if circuit breaker triggered or max rounds reached
        if force_consensus or current_round >= max_rounds:
            if not consensus_reached:
                logger.warning(f"⚠️ Forcing consensus: Circuit breaker={force_consensus}, Round={current_round}/{max_rounds}")
                consensus_reached = True
        ...
        if consensus_reached:
            ...
            # Create investment plan prompt
            plan_prompt = ChatPromptTemplate.from_messages([
                ("system", """You are a senior investment strategist creating a final investment recommendation.
...
"""),
                ("user", """Based on all the analysis and debate, create a comprehensive investment plan.
...
"""
            ...
            debate_state["judge_feedback_for_next_round"] = judge_feedback
            
            return {
                "research_debate_state": debate_state,
                "investment_plan": None  # Explicitly set to None
            }
