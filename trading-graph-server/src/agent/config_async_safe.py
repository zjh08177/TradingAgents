"""
Async-Safe Trading Configuration System
========================================
COMPLETELY ASYNC-SAFE: No file I/O at import time, all operations are lazy.
"""
import os
import asyncio
...
class TradingConfig(BaseSettings):
    ...
    deep_think_model: str = Field(default="gpt-4o", env="DEEP_THINK_MODEL")
    quick_think_model: str = Field(default="gpt-4o-mini", env="QUICK_THINK_MODEL")
    backend_url: str = Field(default="https://api.openai.com/v1", env="BACKEND_URL")
...
def get_trading_config() -> TradingConfig:
    ...
    COMPLETELY ASYNC-SAFE: No file I/O, no dotenv loading.
    Only reads from environment variables that are already set.
    """
    global _config
    if _config is None:
        # Create config WITHOUT loading .env file
        # Pydantic will read from environment variables that are already set
